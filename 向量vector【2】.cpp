 ...
 const char * msg_to_user( int num_tries )  {
    ...
    //静态常量的字符指针，指向常量的指针const int * p=&a;指针类型的常量int * const p=&a;
      static const char * user_msgs[ rap_cnt ] = {
        "Go on, make a guess.","Oops! Nice guess but not quite it.",
        "Hmm. Sorry. Wrong again!", "Ah, this is harder than it looks ,no ? ",
...
int main()  {
  ...
  //  vector<int>elem_seq( seq_size, elem_vals );

  //  vector<int> *seq_addrs[ seq_cnt ] = { &fibonacci, &lucas, &pell,
        //            &triangular, &square, &pentagonal };

  //   srand( seq_cnt);      //使用seq_cnt作为随机函数的种子
  ...
  //  current_vec = seq_addrs[ seq_index ];
    ...
    while(  cur_tuple < seq_size )  {
      ...
      cin >> user_guss;
      if(user_guss == elem_vals[ cur_tuple + 2 ])  {
